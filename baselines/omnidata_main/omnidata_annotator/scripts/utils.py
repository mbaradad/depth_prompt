...
def create_camera(location, rotation,
    ...
    camera_data = bpy.data.camera_type.new(camera_name)
...
def get_or_create_camera(location, rotation,
    ...
    if camera_name in bpy.data.camera_type:
        camera_data = bpy.data.camera_type[camera_name]
    else:
        camera_data = bpy.data.camera_type.new(camera_name)
...
def delete_objects_starting_with(prefix):
    ...
    for obj in bpy.data.objects:
        if obj.name.startswith(prefix):
            obj.select = True
            if obj.type == 'PERSP':
                bpy.data.camera_type.remove(obj.data)
...
def purge_all():
    cameras = bpy.data.camera_type
    for c in cameras:
        if c.users == 0:
            bpy.data.camera_type.remove(c)
