...
parser.add_argument('--num_quantiles', dest='num_quantiles', help="number of qualtiles to use in blur stack. More is better, but slower", required=False)
...
parser.add_argument('--input_path', dest='input_path', help="path to folder containing rgb and depth_euclidean")
parser.set_defaults(input_path='/data/vision/torralba/movies_sfm/projects/normals_acc/data/test_images/google_scans/Schleich_Spinosaurus_Action_Figure_texture_04.jpg')

parser.add_argument('--output_path', dest='output_path', help="path to where refocused rgb should be stored")
parser.set_defaults(output_path='/tmp')
...
def save_outputs(img_path, output_file_name):
    if not output_file_name.__contains__('rgb'):
        print("Invalid file: {}".format(img_path))
        return
    rgb_path = img_path
...
if input_path.is_dir():
    ...
    save_outputs(str(input_path), os.path.splitext(os.path.basename(input_path))[0])