...
from baselines.leres.LeReS.Minist_Test.lib.test_utils import refine_focal, refine_shift
...
from baselines.leres.LeReS.Minist_Test.lib.spvcnn_classsification import SPVCNN_CLASSIFICATION
from baselines.leres.LeReS.Minist_Test.lib.test_utils import reconstruct_depth
...
if __name__ == '__main__':
    ...
    image_dir = '/data/vision/torralba/movies_sfm/projects/normals_acc/data/test_images/google_scans'
    imgs_list = os.listdir(image_dir)
    imgs_list.sort()
    imgs_path = [os.path.join(image_dir, i) for i in imgs_list if i != 'outputs' and i != 'temp']
    ...
    for i, v in enumerate(imgs_path):
        ...
        pcd = reconstruct_depth(depth_scaleinv, rgb[:, :, ::-1], image_dir_out, img_name[:-4]+'-pcd', focal=focal_length)

        pcd = pcd.reshape(rgb.shape)

        show_pointcloud(pcd.transpose((2,0,1)), rgb.transpose((2,0,1))[::-1])